...
outfileCov = gzip.open("/staging/leuven/stg_00019/research/Kate/fragmentome/results/body/%s/fft_summaries/fft_%s_cov.tsv.gz"%(ind,ind),'w')
outfileStarts = gzip.open("/staging/leuven/stg_00019/research/Kate/fragmentome/results/body/%s/fft_summaries/fft_%s_starts.tsv.gz"%(ind,ind),'w')
outfileWPS = gzip.open("/staging/leuven/stg_00019/research/Kate/fragmentome/results/body/%s/fft_summaries/fft_%s_WPS.tsv.gz"%(ind,ind),'w')
...
for cid in sorted(geneIDs):
  if os.path.exists("/staging/leuven/stg_00019/research/Kate/fragmentome/results/body/%s/fft/block_%s.tsv.gz"%(ind,cid)):
    if not wrote_header:
      header = ['#Region']
      infile = gzip.open("/staging/leuven/stg_00019/research/Kate/fragmentome/results/body/%s/fft/block_%s.tsv.gz"%(ind,cid))
      infile.readline()
      for line in infile:
        fields = line.split()
